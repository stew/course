...
spaces ::
  Parser Chars
spaces = spaces1 ||| valueParser Nil
...
hex ::
  Parser Char
hex = result <$> readHex <$> ((is 'u') *> (replicateA 4 (satisfy isHexDigit))) where
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby pa ps = sepby1 pa ps ||| (valueParser Nil)
...
eof ::
  Parser ()
eof = P { parse = eof' } where
  eof' Nil = Result Nil ()
  eof' (h:._) = UnexpectedChar h
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll preds = P { parse = satisfyAll' } where
  satisfyAll' Nil = UnexpectedEof
  satisfyAll' (h:.t)
    | and $ (\f -> f h) <$> preds = Result t h
    | otherwise = UnexpectedChar h
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny preds = P { parse = satisfyAny' } where
  satisfyAny' Nil = UnexpectedEof
  satisfyAny' (h:.t)
    | or $ (\f -> f h) <$> preds = Result t h
    | otherwise = UnexpectedChar h 
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma o c pa = betweenCharTok o c (sepby pa (charTok ','))
